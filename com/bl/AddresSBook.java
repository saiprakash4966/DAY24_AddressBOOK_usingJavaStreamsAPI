...
public class AddresSBook
{
	...
	public void searchByCity(String city) {
		List<Contact> collect = contactDetailsList.stream().filter(p -> p.getCity().equalsIgnoreCase(city))
				.collect(Collectors.toList());
		for (Contact contact : collect) {
			System.out.println("Search result: " + contact);
		}
	...
	public void searchByState(String state) {
		List<Contact> collect = contactDetailsList.stream().filter(p -> p.getCity().equalsIgnoreCase(state))
				.collect(Collectors.toList());
		for (Contact contact : collect) {
			System.out.println("Search result: " + contact);
		}
